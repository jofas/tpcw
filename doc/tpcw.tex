...
iteration $i$.
The consequence is, that for small $i$, $\jmax(i) = n$
more often, which means that the first iterations are
...
% describe phase one results

% critical section 1: all schedules just minor differences

% critical section 2: under 3s: just dyn, 1-16, stat, 2-8

% static 2-8 majorly outperform static

% chunk size has big influence (see static, dyn)
% -> except guided
...
% discussion: answer questions raised in results
